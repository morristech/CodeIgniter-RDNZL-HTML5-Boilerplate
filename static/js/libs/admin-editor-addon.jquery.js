...
$.fn.attachAdmin = function(options) {
	...
	if(!options){
		...
		options.backEndElements = (!options.backEndElements) ? [] : options.backEndElements;
		options.frontEndElements = (!options.frontEndElements) ? [] : options.frontEndElements;
		options.handlerURLs = (!options.handlerURLs) ? [] : options.handlerURLs;
	}
				...
				if(el_data.field_type === 'text' || el_data.field_type === 'datetime' || el_data.field_type === 'int'){
					...
					if(!el_data.field_options){
						...
						$.each(el_data.field_options, function(z, enum_opt){
							var $option = $('<option/>').val(enum_opt).text(enum_opt)
								.appendTo($field);
						});
					}
				}
